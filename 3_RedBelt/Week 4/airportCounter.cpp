...
class AirportCounter {
   private:
    int AirAsInt(TAirport air) const { return static_cast<int>(air); }
    TAirport IntAsAir(int index) const { return static_cast<TAirport>(index); }
    void IncrementAir(TAirport& air) { air = IntAsAir(AirAsInt(air) + 1); }
    size_t airsSize() const { return static_cast<size_t>(TAirport::Last_); }

   public:
    // конструктор по умолчанию: список элементов пока пуст
    AirportCounter() {
        TAirport curAir = static_cast<TAirport>(0);
        while (curAir != TAirport::Last_) {
            airportFlies_[AirAsInt(curAir)].first = curAir;
            airportFlies_[AirAsInt(curAir)].second = 0;
            IncrementAir(curAir);
        }
    }
    ...
    AirportCounter(TIterator begin, TIterator end) : AirportCounter() {
        while (begin != end) {
            ++airportFlies_[AirAsInt(*begin++)].second;
        }
    }

    // получить количество элементов, равных данному
    size_t Get(TAirport airport) const {
        if (AirAsInt(airport) >= airsSize()) {
            return 0;
        }
        return airportFlies_.at(AirAsInt(airport)).second;
    }

    // добавить данный элемент
    void Insert(TAirport airport) { ++airportFlies_[AirAsInt(airport)].second; }

    // удалить одно вхождение данного элемента
    void EraseOne(TAirport airport) { --airportFlies_[AirAsInt(airport)].second; }

    // удалить все вхождения данного элемента
    void EraseAll(TAirport airport) { airportFlies_[AirAsInt(airport)].second = 0; }

    using Item = pair<TAirport, size_t>;
    using Items = std::array<pair<TAirport, int>, static_cast<size_t>(TAirport::Last_)>;

    // получить некоторый объект, по которому можно проитерироваться,
    // получив набор объектов типа Item - пар (аэропорт, количество),
    // упорядоченных по аэропорту
    Items GetItems() const { return airportFlies_; }
...
#define MANYQUERIES 1000'000'000
void TestManyConstructions() {
    ...
    uint64_t total = 0;
    for (int step = 0; step < MANYQUERIES; ++step) {
...
void TestManyGetItems() {
    ...
    uniform_int_distribution<size_t> gen_airport(0,
...
void TestMostPopularAirport() {
    ...
    ASSERT(all_of(begin(most_popular), end(most_popular),
